    ...
    <div class="row">
        <div class="col-12">
            ...
            <div class="page-header rounded mb-4">
                <div class="container">
                    <div class="d-flex justify-content-between align-items-center">
                        <div>
                            <h1 class="h3 mb-0">Editar Cotización</h1>
                            <p class="mb-0 opacity-75">Modifica los datos de la cotización: {{ $quote->name }}</p>
                        </div>
                        <div class="btn-group" role="group">
                            <a href="{{ route('quotes.show', $quote->_id) }}" class="btn btn-outline-primary">
            ...
            <div class="card shadow">
                <div class="card-body">
                    <form action="{{ route('quotes.update', $quote->_id) }}" method="POST" id="quoteForm">
                        @csrf
                        @method('PUT')
                                    ...
                                    <div class="card-body">
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <input type="text" class="form-control @error('name') is-invalid @enderror" 
                                                id="name" name="name" value="{{ old('name', $quote->name) }}" required>
                                            @error('name')
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <input type="text" class="form-control @error('zone') is-invalid @enderror" 
                                                id="zone" name="zone" value="{{ old('zone', $quote->zone) }}" required>
                                            @error('zone')
                                                <div class="invalid-feedback">{{ $message }}</div>
                                            @enderror
                                            ...
                                            <select class="form-select @error('clientId') is-invalid @enderror" 
                                                ...
                                                @foreach($clients as $client)
                                                    <option value="{{ $client->_id }}" 
                                                        {{ old('clientId', $quote->clientId) == $client->_id ? 'selected' : '' }}>
                                                        {{ $client->name }}
                                                    </option>
                                                @endforeach
                                            </select>
                                                ...
                                                @foreach($files as $file)
                                                    @if($file->type === 'pdf')
                                                        <option value="{{ $file->_id }}" 
                                                            {{ old('fileId', $quote->fileId) == $file->_id ? 'selected' : '' }}>
                                                            {{ $file->name }}
                                                        </option>
                                                    @endif
                                                @endforeach
                                            ...
                                            <select class="form-select @error('responsibleId') is-invalid @enderror" 
                                                ...
                                                @foreach($users as $user)
                                                    <option value="{{ $user->_id }}" 
                                                        {{ old('responsibleId', $quote->responsibleId) == $user->_id ? 'selected' : '' }}>
                                                        {{ $user->name }}
                                                    </option>
                                                @endforeach
                                            </select>
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <select class="form-select @error('status') is-invalid @enderror" 
                                                ...
                                                <option value="pending" {{ old('status', $quote->status) == 'pending' ? 'selected' : '' }}>Pendiente</option>
                                                <option value="approved" {{ old('status', $quote->status) == 'approved' ? 'selected' : '' }}>Aprobada</option>
                                                <option value="rejected" {{ old('status', $quote->status) == 'rejected' ? 'selected' : '' }}>Rechazada</option>
                                                <option value="active" {{ old('status', $quote->status) == 'active' ? 'selected' : '' }}>Activa</option>
                                                <option value="completed" {{ old('status', $quote->status) == 'completed' ? 'selected' : '' }}>Completada</option>
                                            </select>
                                            @error('status')
                                                <div class="invalid-feedback">{{ $message }}</div>
                                            @enderror
                                ...
                                <div class="card">
                                    ...
                                    <div class="card-body">
                                        <div id="cranesContainer">
                                            <!-- Las grúas existentes se cargarán aquí -->
                                        </div>
                                        ...
                                        <div class="mb-3 mt-4">
                                            ...
                                            <input type="number" class="form-control @error('iva') is-invalid @enderror" 
                                                id="iva" name="iva" value="{{ old('iva', $quote->iva) }}" min="0" max="100" step="0.01">
                                            @error('iva')
                                                <div class="invalid-feedback">{{ $message }}</div>
                                            @enderror
                                        </div>
                                        ...
                                        <div class="card bg-light mt-3">
                                            <div class="card-body">
                                                ...
                                                <div class="d-flex justify-content-between mb-2">
                                                    <span>IVA (<span id="ivaRateDisplay">{{ $quote->iva ?? 16 }}</span>%):</span>
                                                ...
                                                <input type="hidden" id="total" name="total" value="{{ $quote->total }}">
                                            </div>
                                        </div>
                                    </div>
                                </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Datos existentes de la cotización
        const existingCranes = @json($quote->cranes);
        ...
        function addCrane(existingData = null) {
            ...
            selects.forEach(select => {
                ...
                // Establecer valor existente
                if (existingData && select.classList.contains('crane-select')) {
                    select.value = existingData.crane;
                }
            });
            ...
            inputs.forEach(input => {
                ...
                if (existingData) {
                    if (input.classList.contains('dias-input')) {
                        input.value = existingData.dias;
                    } else if (input.classList.contains('precio-input')) {
                        input.value = existingData.precio;
                    }
                }
            });
            ...
            // Actualizar subtotal inicial si hay datos existentes
            if (existingData) {
                const craneItem = cranesContainer.lastElementChild;
        ...
        function updatePriceBasedOnZone(select, zone) {
            ...
            if (price && price > 0 && !precioInput.value) {
        ...
        // Cargar grúas existentes
        if (existingCranes && existingCranes.length > 0) {
            existingCranes.forEach(crane => {
                addCrane(crane);
            });
        } else {
            // Agregar una grúa vacía si no hay grúas existentes
            addCrane();
        }
